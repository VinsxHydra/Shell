...
// SESSION clipboard simpan sementara
...
// Fungsi zip rekursif
function zipRecursive($source, &$zip, $base = '') {
    if (is_dir($source)) {
        foreach (scandir($source) as $f) {
            if ($f === '.' || $f === '..') continue;
            zipRecursive("$source/$f", $zip, "$base/$f");
        }
    } else {
        $zip->addFile($source, ltrim($base, '/'));
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    switch ($action) {
        ...
        case 'copy':
            $_SESSION['clipboard'] = $selected;
            break;

        case 'paste':
            foreach ($_SESSION['clipboard'] as $item) {
                $basename = basename($item);
                $dest = $path . DIRECTORY_SEPARATOR . $basename;
                if (is_dir($item)) {
                    $it = new RecursiveIteratorIterator(
                        new RecursiveDirectoryIterator($item, RecursiveDirectoryIterator::SKIP_DOTS),
                        RecursiveIteratorIterator::SELF_FIRST
                    );
                    foreach ($it as $src => $info) {
                        $target = $dest . DIRECTORY_SEPARATOR . substr($src, strlen($item) + 1);
                        if ($info->isDir()) mkdir($target, 0777, true);
            ...
            $_SESSION['clipboard'] = []; // clear if needed
            break;

        case 'zip':
            $zipPath = $path . "/archive_" . time() . ".zip";
            $zip = new ZipArchive();
            if ($zip->open($zipPath, ZipArchive::CREATE)) {
                foreach ($selected as $item) {
                    zipRecursive($item, $zip, basename($item));
                }
                $zip->close();
            }
            break;
        ...
        case 'unzip':
            foreach ($selected as $item) {
                if (is_file($item) && preg_match('/\.zip$/i', $item)) {
                    $zip = new ZipArchive();
                    if ($zip->open($item)) {
                        $zip->extractTo($path);
                        $zip->close();
...
<div class="mt-4 flex items-center space-x-4">
    <select name="action" class="bg-black text-white border border-gray-600 p-2">
    ...
    <button type="submit" class="bg-green-500 text-black font-bold px-4 py-1 rounded">➤</button>
</div>
</div></body></html>
